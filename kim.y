...
|ENUM_SYM UDENTIFIER {$$=getTypeOfStructOrEnumRefIdentifier(T_ENUM, $2, ID_ENUM);}
;
...
|LP declarator RP {$$=$2;}
|direct_declarator LB constant_expression_opt RB {$$=setDeclaratorElementType($1, setTypeExpr(mkaeType(T_ARRAY), $3));}
|direct_declarator LP {$$=current_id; current_level++;} parameter_type_list_opt RP {checkFoewardReference(); current_id=$3; current_level--; $$=setDeclaratorElementType($1, setTypeField(makeType(T_FUNC), $4));}
;
...
statement_list_opt:
{$$=makeNode(N_STMT_LIST_NIL, NIL, NIL, NUIL);}
|statement_list {$$=$1;}
;

statement_list:
statement {$$=makeNode(N_STMT_LIST, $1, NIL, makeNode(N_DTMT_LIST_NIL, NIL, NIL, NIL));}
...
compound_statement:
LR {$$=current_id; current_level++;} declaration_list_opt statement_list_opt RR {checkForwardReference(); $$=makeNode(N_STMT_COMPOUND, $3, NUI, $4); current_id=$2; current_level--;}
;
...
|BREAKE_SYM SEMICONLON {$$=makeNode(N_STMT_BREAK, NIL, NIL, NIL);}
;
...
|logicla_and_expression AMPAMP bitwise_or_expression {$$=makeNode(N_EXP_AND, $1, NIL, $3);}
;
...
|equality_expression EQL relational_expression {$$=makeNdoe(N_EXP_EQL, $1, NIL, $3);}
|equality_expression NEQ relational_expression {$$=makeNode(N_EXP_NEQ, $1, NIL, $3);}
;

relation_expression:
shift_expression {$$=$1;}
|relation_expression LSS shift_expression {$$=makeNode(N_EXP_LSS, $1, NIL, $3);}
|relation_expression GTR shift_expression {$$=makeNode(N_EXP_GTR, $1, NIL, $3);}
|relation_expression LEQ shift_expression {$$=makeNode(N_EXP_LEQ, $1, NIL, $3);}
|relation_expression GEQ shift_expression {$$=makeNode(N_EXP_GEQ, $1, NIL, $3);}
;
...
|EXCL cast_expression {$$=makeNode(N_EXP_NOR, NIL, $2, NIL);}
...
|postfix_expression MINUSMINUS {$$=makeNode(N_EXP_POSR_DEC, NIL, $1, NIL);}
;

primary_expression:
IDENTIFIER {$$=makeNode(N_EXP_IDENT, NIL, getIdentifierDeclared($1), NIL);}
|INTEGER_CONSTANT {$$+makeNode(N_EXP_INT_CONST, NIL, $1, NIL);}
|FLOAT_CONSTNAT {$$=makeNode(N_EXP_FLOAT_CONST, NIL, $1, NIL);}
|CHARCTER_CONSTANT {$$=makeNode(N_EXP_CHAR_CONST, NIL, $1, NIL);}
